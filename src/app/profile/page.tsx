        ...
        <aside className="w-[200px] shrink-0">
          <div className="flex flex-col items-center">
            <div className="relative rounded-full size-[96px] overflow-hidden border-4 border-white shadow-sm">
              <img alt="Avatar" className="w-full h-full object-cover" src="/images/ProfilePage/d4699efb0b0581a2c8ec625c4639f0d9a00865fa.png" />
            </div>
          ...
          <nav className="mt-8 space-y-1">
            ...
            <button
              onClick={() => logout()}
              className="w-full flex items-center gap-3 px-4 py-3 rounded-[8px] border-none cursor-pointer text-left font-['Inter',sans-serif] text-[14px] text-[#df0000] tracking-[-0.42px] bg-transparent hover:bg-red-50 mt-4"
            >
              <LogOut size={16} />
              Logout
            </button>
            ...
            <div className="mt-6 border border-[#d9d9d9] rounded-[4px] overflow-hidden">
              ...
              {orderHistory.map((order) => (
                <div key={order.id} className="grid grid-cols-5 px-6 py-4 border-t border-[#ececec] items-center">
                  ...
                  <span className={`font-['Inter:Bold',sans-serif] font-bold text-[12px] tracking-[0.5px] ${order.status_color}`}>{order.status}</span>
                  <span className="font-['Inter',sans-serif] text-[14px] text-black tracking-[-0.42px]">{order.total_price}</span>
