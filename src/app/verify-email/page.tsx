...
export default function VerifyEmailPage() {
  ...
  const canSubmit = /^\S+@\S+\.\S+$/.test(email) && /^\d{6}$/.test(token);
      ...
      <div className="flex max-w-[906px] w-full shadow-lg">
        ...
        <div className="bg-white flex-1 p-8 sm:p-16 flex flex-col justify-center min-h-[675px]">
          ...
          <form onSubmit={handleVerify} className="mt-8 space-y-5">
            <div>
              ...
              <input
                ...
                className="w-full mt-2 bg-[#FDF9F5] border border-[#d8d0c8] rounded-[8px] px-4 py-3 font-['Manrope',sans-serif] text-[14px] outline-none"
              />
            </div>

            <div>
              <label className="font-['Manrope',sans-serif] text-[12px] text-[#605850] tracking-[1.1px] uppercase">Verification Code</label>
              <InputOTP value={token} onChange={setToken} maxLength={6} containerClassName="mt-3 justify-center sm:justify-start">
                <InputOTPGroup>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mt-5 font-['Manrope',sans-serif] text-[13px]">
            <button type="button" onClick={handleResend} disabled={resending} className="bg-transparent border-none text-[#a24141] cursor-pointer disabled:opacity-60">
              {resending ? "Sending..." : "Resend code"}
            </button>
            <span className="hidden sm:inline text-[#d8d0c8]">|</span>
            <Link href="/register" className="text-[#a24141] no-underline">Change email</Link>
            <span className="hidden sm:inline text-[#d8d0c8]">|</span>
